package com.gh.lazy.utils.common
 ...
 * @param action 执行方法
 */
var clickTagKey = 264166231 //随便设置的一个数
fun View.clickNoRepeat(interval: Long = 500, action: (view: View) -> Unit) {
    setOnClickListener {
        val currentTime = System.currentTimeMillis()
        val tag = getTag(clickTagKey)
        val lastClickTime = if (tag != null && tag is Long) {
            tag
        } else {
            0
        }
        ...
        setTag(clickTagKey, currentTime)
        action(it)
    }
}
